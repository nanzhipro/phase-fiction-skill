...
- 本文件用于约束 AI 在《草原列车疑云》仓库中的 phase 执行行为，对 Copilot / Claude Code / Codex 三方等效生效。
...
5. 拿到 `finalize` 输出后，必须做一次深入审视，不得只复述结果。
6. 最终执行仪表盘至少应覆盖：项目总览、Phase 台账、仓库状态、Health 检查、风险与遗留、推荐人类下一步。
